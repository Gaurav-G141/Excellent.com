    ...
    fields: [
      {
        ...
        expected,
        meaning: `how fast ${theme.quantityName} is rising right when ${theme.symbol} = ${u0}, in extra ${theme.outputUnit} for each one-unit increase in ${theme.inputName}`,
      },
    ],
    ...
    fields: [
      {
        ...
        expected: average,
        meaning: `how fast ${theme.quantity} changed on average between x = ${p} and x = ${q} seconds, in ${theme.unit} per second`,
      },
      {
        kind: 'number',
        label: 'The single moment it equals that (x = ?)',
        expected: moment,
        meaning: `the single value of x (in seconds) where ${theme.quantity} is changing at exactly that average pace`,
      },
    ],
